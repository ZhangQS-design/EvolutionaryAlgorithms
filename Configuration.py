...
# sub_task的属性配置
# upload_size_range = (0.5, 1.5)    # M
upload_size_range = (0.5, 1.5)    # M
# download_size_range = (5, 15)   # M
download_size_range = (5, 15)   # M
# compute_cost_range = (1, 10)  # 10**9
compute_cost_range = (1, 10)  # 10**9
...
vehicles_attrs = pd.read_csv('data/vehicles_attr.csv', header=None, index_col=None)
fogs_attrs = pd.read_csv('data/fogs_attr.csv', header=None, index_col=None)
uavs_attrs = pd.read_csv('data/uavs_attr.csv', header=None, index_col=None)

# 所有节点的存储资源 + UAV
origin_source = np.concatenate((vehicles_attrs.iloc[:, 2].values, fogs_attrs.iloc[:, 2].values,
                                np.array([cloud_memory]), uavs_attrs.iloc[:, 2].values))
# 所有节点的存储资源
# origin_source = np.concatenate((vehicles_attrs.iloc[:, 2].values, fogs_attrs.iloc[:, 2].values,
#                                 np.array([cloud_memory])))
# print("points")

# 先暂时不考虑 100个无人机伴飞
# origin_source = np.concatenate((vehicles_attrs.iloc[:, 2].values, fogs_attrs.iloc[:, 2].values,
#                                 np.array([cloud_memory]), uavs_attrs.iloc[:, 2].values))
# origin_source = np.concatenate((vehicles_attrs.iloc[:, 2].values, fogs_attrs.iloc[:, 2].values, np.array([cloud_memory])))
# print(origin_source)