...
from Configuration import fog_compute_power_range, fog_trans_rate_range, fog_memory_range, v_compute_power_range, \
    ...
    vehicles_attr, data_uav, u2vf_limit_distance, cloud_compute_ability, uav_compute_power_range, uav_trans_rate_range, \
    uav_memory_range
...
def get_info_by_node_id(nid):
    ...
    if 0 <= nid <= 99:
        ...
        df = pd.read_csv('data/uavs_attr.csv', header=None, index_col=None)
...
def get_node_memory():
    # print(nid)
    mem_array = np.zeros(111, dtype=int)
    # print(mem_array)
    for nid in range(0, 111):
        if 0 <= nid <= 99:
            df = pd.read_csv('data/vehicles_attr.csv', header=None, index_col=None)
            ...
            df = pd.read_csv('data/uavs_attr.csv', header=None, index_col=None)
            mem_array[nid] = df.iloc[nid - 105, 2]
    return mem_array
...
def compute_delay_in_a_link(redeay_server_time, chromosomes, chromosome_id):
        ...
        if len(nodes_loads[n_id]):  # 只考虑有负载的节点
            if n_id != 104:
                ...
                for n_i_sub in nodes_loads[n_id]:
                    # if n_i_sub.belonging_vehicle_id == n_id:  # 如交付节点是车辆自身，则上传时延为0
                    #     n_i_sub.up_delay = 0
                    if n_i_sub.belonging_uav_id == n_id:  # 如交付节点是车辆自身，则上传时延为0
                        n_i_sub.up_delay = 0
                    else:
                        # 否则由该节点上的所有子任务等分节点的传输速率，按照上传数据大小计算时间
                        n_i_sub.up_delay = n_i_sub.upload_size / (n_tr / nodes_loads[n_id].__len__())
                ...
                for z in range(tr_rank_loads.__len__()):
                    if z == 0:
                        ...
                        is_reachable = calculate_two_vehicle_is_interact(
                            ...
                            tr_rank_loads[z].belonging_vehicle_id, tr_rank_loads[z].allocate_node_id)
                        if not is_reachable:
                            tr_rank_loads[z].punish_delay = 0.1
                        else:
                            tr_rank_loads[z].punish_delay = 0

                    else:
                        # 后续到达的任务，总的时延为：自身传输时延与上一个子任务的传输时延与计算时延之和中的较大者 + 该子任务的计算时延
                        if tr_rank_loads[z - 1].up_delay + tr_rank_loads[z - 1].compute_delay \
                                > tr_rank_loads[z].up_delay:
                            tr_rank_loads[z].wait_delay = tr_rank_loads[z - 1].up_delay + \
                                                          tr_rank_loads[z - 1].compute_delay - tr_rank_loads[z].up_delay
                        else:
                            tr_rank_loads[z].wait_delay = 0
                    ...
                    if tr_rank_loads[z].belonging_vehicle_id == n_id:
                        tr_rank_loads[z].down_delay = 0
                    else:
                        _, object_vehicle_transmission, _ = \
                            get_info_by_node_id(tr_rank_loads[z].belonging_vehicle_id)
                        tr_rank_loads[z].down_delay = tr_rank_loads[z].upload_size / object_vehicle_transmission
                    tr_rank_loads[z].whole_delay = tr_rank_loads[z].up_delay + tr_rank_loads[z].wait_delay + \
                                                   tr_rank_loads[z].compute_delay + tr_rank_loads[z].punish_delay +\
                                                   tr_rank_loads[z].down_delay
                ...
                # 云节点的处理
                for n_i_sub in nodes_loads[n_id]:
                    n_i_sub.up_delay = n_i_sub.upload_size / (cloud_transmission_rate / nodes_loads[n_id].__len__())
                nodes_loads[n_id].sort(key=lambda x: x.upload_size)
                tr_rank_loads = nodes_loads[n_id]
                ...
                for z in range(tr_rank_loads.__len__()):
                    ...
                    tr_rank_loads[z].compute_delay = tr_rank_loads[z].compute_cost / cloud_compute_ability
                    tr_rank_loads[z].punish_delay = 0
                    tr_rank_loads[z].whole_delay = tr_rank_loads[z].up_delay + tr_rank_loads[z].wait_delay + \
                                                   tr_rank_loads[z].compute_delay + tr_rank_loads[z].punish_delay + \
                                                   tr_rank_loads[z].down_delay
    ...
    for z in task_set:
        ...
        for zz in z:
            ...
            task_num_in_a_link += 1
    print("a_link_delays", a_link_delays)
    # print("计算时延时的信息：----------->")
    # print("平均任务时延：")
    # print(sum(a_link_delays) / task_num_in_a_link)
    # print("最大任务时延：")
    ...
    return delay_info, task_average_comp_delay, task_average_trans_delay, task_average_wait_delay, task_average_delay
...
def initialization(dispatch_pre_time, dispatch_time):
    ...
    if len(sure_subtasks) != 0:
        ...
        delay_info_dict, comp, trans, wait, all_delay = compute_delay_in_a_link(dispatch_time, [initial_seeds], 0)  # 计算时延信息
        initial_genetic_delays.append(all_delay)
        initial_genetic_comp_delays.append(comp)
        initial_genetic_trans_delays.append(trans)
        initial_genetic_wait_delays.append(wait)
    ...
    return initial_seeds, initial_genetic_delays, initial_hop_delays, initial_genetic_comp_delays, \
        initial_genetic_trans_delays, initial_genetic_wait_delays
